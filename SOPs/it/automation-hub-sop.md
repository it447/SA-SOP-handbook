...
It's hosted on Vercel (serverless Python functions), with Upstash Redis as the only persistent state layer — there is no separate database.

## Who this is for

IT / Ops team members managing the outbound automation pipeline (HubSpot → Instantly, Clay, Google Sheets, and related integrations).
...
- Function timeout is 10 seconds for most routes. The sync endpoint responds immediately with `{"ok": true}` then runs the sync in the background to avoid timeout issues.
...
Each automation is stored as a JSON object inside the `automations_config` array:
...
4. **Load sent cache** — performs a single Redis SCAN of all `sent:*` keys into memory. This avoids making one Redis call per contact.
5. **Loop over each automation:**
   - **Enrichment type:** Runs Clay push and/or GSheet pull, then continues to next automation.
   - **Instantly/form type:**
...
Filters are applied after the delay check, before the API call. Each filter specifies:
...
## Dashboard API (`api/automations.py`)
...
### The debug endpoint (`/api/instantly/debug?campaign_id=...`)
...
Returns an `action_needed` field summarizing what is wrong.
...
### Google Sheet → HubSpot enrichment (enrichment automations)
...
### `/api/sync_gsheet` → `api/sync_gsheet.py`
...
When `slack_enabled: true` on an automation, every successful enrollment sends a message to `slack_channel`. The message uses a template with variables: `{{email}}`, `{{first_name}}`, `{{last_name}}`, `{{company}}`.
